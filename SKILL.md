...
- **Dark theme (Nightfall) / Light theme (Daybreak):** warm dark earth, not blue-black. Daybreak is warm paper and diffuse fieldlight. Auto under `prefers-color-scheme`; force with `<html data-theme="dark">` / `data-theme="light">` ("light" = Daybreak). Use semantic tokens (incl. `--on-*-soft`, `--control-track`, `--hover-wash`, `--text-on-inverse`) so your UI flips for free; reach for fixed scale steps (`--stone-900`) only for moments meant to stay dark in both themes.
